...
import { TrendingUp, TrendingDown, Plus, RefreshCw, ArrowUpRight } from "lucide-react";

interface Holding {
  id: string; symbol: string; name: string; quantity: number;
  avgBuyPrice: number; currentPrice: number;
  color: string; bg: string; letter: string; sector: string;
}
...
const PORTFOLIO_HISTORY = [
  {t:"Aug",v:72000},{t:"Sep",v:74500},{t:"Oct",v:78000},{t:"Nov",v:81000},{t:"Dec",v:86000},
  {t:"Jan",v:84000},{t:"Feb",v:88000},{t:"Mar",v:91000},{t:"Apr",v:90200},{t:"May",v:93314},
];
...
function mv(h:Holding)     { return h.currentPrice*h.quantity; }
...
function StockAvatar({symbol,color,bg,letter,size=36}:{symbol:string;color:string;bg:string;letter:string;size?:number}) {
  ...
  const clean=symbol.replace(".BSE","").replace("/","").slice(0,6);
  const domain=LOGO_DOMAINS[clean];
  return (
    <div style={{width:size,height:size,borderRadius:"50%",background:bg,border:`1px solid ${color}33`,
      display:"flex",alignItems:"center",justifyContent:"center",fontSize:size*0.32,fontWeight:700,color,flexShrink:0,overflow:"hidden"}}>
      {domain&&!err
...
export default function PortfolioPage() {
  ...
  const loadHoldings=useCallback(async()=>{
    ...
    try {
      ...
      const res=await fetch(`/api/holdings?portfolioId=${me.portfolioId}`,{headers:{Authorization:`Bearer ${token}`}});
      if(!res.ok) throw new Error("holdings");
      const data:Holding[]=await res.json();
      if(data?.length>0){
        const enriched=await Promise.all(data.map(async h=>{
          try{
            const qRes=await fetch(`/api/stocks/${h.symbol}?market=${market.id}`);
            if(!qRes.ok) return h;
            const q=await qRes.json();
            return {...h,currentPrice:typeof q.price==="number"?q.price:h.currentPrice};
          }catch{return h;}
        }));
  ...
  const sectorData=sectors.map((s,i)=>({
    name:s,
    value:holdings.filter(h=>h.sector===s).reduce((a,h)=>a+mv(h),0),
  ...
  const displayed=holdings
    ...
    .sort((a,b)=>sortBy==="value"?mv(b)-mv(a):sortBy==="pnl"?pnl(b)-pnl(a):pnlPct(b)-pnlPct(a));

  const fmt=(n:number)=>Math.abs(n)>=1000
    ...
    <div style={{padding:"24px 32px",maxWidth:1200,margin:"0 auto",fontFamily:"'Geist','Inter',sans-serif",background:C.page,minHeight:"100vh"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24}}>
        ...
        <div style={{display:"flex",gap:10}}>
          <button onClick={loadHoldings} style={{width:36,height:36,borderRadius:10,border:`1px solid ${C.line}`,background:"transparent",color:C.muted,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <RefreshCw size={14}/>
          </button>
          <button onClick={()=>router.push("/dashboard")} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 18px",background:"#8FFFD6",borderRadius:10,border:"none",cursor:"pointer",color:"#0a0a0a",fontWeight:700,fontSize:13}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1.4fr 1fr 1fr 1fr",gap:14,marginBottom:24}}>
        ...
        {[
          {label:"Total Cost",     value:`${currency}${totalCost.toLocaleString("en-US",{minimumFractionDigits:2})}`,                                              color:C.muted},
        ...
        <div style={{background:C.card,border:`1px solid ${C.line}`,borderRadius:14,padding:"20px 22px"}}>
          ...
          <ResponsiveContainer width="100%" height={160}>
            <AreaChart data={PORTFOLIO_HISTORY} margin={{top:4,right:4,left:-20,bottom:0}}>
              ...
              <Tooltip contentStyle={{background:C.card,border:`1px solid ${C.line}`,borderRadius:8,fontSize:11}} itemStyle={{color:"#8FFFD6"}} labelStyle={{color:C.muted}}/>
        ...
        <div style={{background:C.card,border:`1px solid ${C.line}`,borderRadius:14,padding:"20px 22px"}}>
          ...
          <ResponsiveContainer width="100%" height={110}>
            <PieChart>
              ...
              <Tooltip contentStyle={{background:C.card,border:`1px solid ${C.line}`,borderRadius:8,fontSize:11}} formatter={(v:any)=>[`${currency}${Number(v).toLocaleString()}`,""]}/>
            </PieChart>
          </ResponsiveContainer>
      ...
      <div style={{background:C.card,border:`1px solid ${C.line}`,borderRadius:14,overflow:"hidden"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 20px",borderBottom:`1px solid ${C.line}`}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <div style={{display:"flex",gap:4}}>
              {allSectors.map(s=>(
                <button key={s} onClick={()=>setFilterSector(s)} style={{padding:"4px 10px",borderRadius:6,border:"none",cursor:"pointer",fontSize:11,background:filterSector===s?"#8FFFD618":"transparent",color:filterSector===s?"#8FFFD6":C.muted,fontWeight:filterSector===s?600:400}}>{s}</button>
              ))}
            </div>
            <div style={{display:"flex",background:C.page,border:`1px solid ${C.line}`,borderRadius:8,padding:3,gap:2}}>
              {(["value","pnl","pnlpct"] as const).map(s=>(
                <button key={s} onClick={()=>setSortBy(s)} style={{padding:"4px 10px",borderRadius:5,border:"none",cursor:"pointer",fontSize:10,fontWeight:600,background:sortBy===s?C.card:"transparent",color:sortBy===s?"#8FFFD6":C.muted}}>
        ...
        {loading?(
          ...
          <div style={{padding:40,textAlign:"center"}}>
            ...
            <p style={{color:C.muted,fontSize:13,margin:"0 0 20px"}}>Start building your portfolio by buying your first stock.</p>
            <button onClick={()=>router.push("/dashboard")} style={{padding:"10px 20px",background:"#8FFFD6",borderRadius:10,border:"none",color:"#0a0a0a",fontWeight:700,fontSize:13,cursor:"pointer"}}>Browse Stocks</button>
          </div>
        ):(
          displayed.map((h,i)=>{
            const pl=pnl(h),plp=pnlPct(h),marketVal=mv(h),up=pl>=0;
            const navSymbol=resolveSymbol(h.symbol,market.id);
            return (
              <div key={h.id}
                onClick={()=>router.push(`/dashboard/stock/${navSymbol}?market=${market.id}`)}
                style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr 1fr 80px",padding:"14px 20px",borderBottom:i<displayed.length-1?`1px solid ${C.line}`:"none",cursor:"pointer",transition:"background 0.15s",alignItems:"center"}}
              ...
              >
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  ...
                  <div>
                    <p style={{color:C.primary,fontWeight:600,fontSize:13,margin:0}}>{h.symbol}</p>
                ...
                <span style={{color:C.muted,fontSize:13}}>{h.quantity.toLocaleString()}</span>
                <span style={{color:C.muted,fontSize:13}}>{currency}{h.avgBuyPrice.toLocaleString("en-US",{minimumFractionDigits:2})}</span>
                ...
                <span style={{color:C.primary,fontSize:13,fontWeight:600}}>{currency}{marketVal.toLocaleString("en-US",{minimumFractionDigits:2})}</span>
                <div>
                  <p style={{color:up?"#22c55e":"#ef4444",fontSize:13,fontWeight:600,margin:0}}>{up?"+":""}{currency}{Math.abs(pl).toLocaleString("en-US",{minimumFractionDigits:2})}</p>
                  <p style={{color:up?"#22c55e":"#ef4444",fontSize:10,margin:"2px 0 0",opacity:0.7}}>{up?"+":""}{plp.toFixed(2)}%</p>
                </div>
                <button
