...
import { Plus, Link2, Trash2, ArrowUpRight, ArrowDownLeft } from "lucide-react";

interface BankAccount { id:string;name:string;bank:string;balance:number;color:string;bg:string;letter:string;linked:boolean;accountNo:string; }
interface FundingEntry { month:string;deposit:number;withdrawal:number; }

const C = {
  page:    "var(--color-page)",
  card:    "var(--color-card)",
  line:    "var(--color-line)",
  lineS:   "var(--color-line-strong)",
...
const FUNDING_HISTORY: FundingEntry[] = [
  {month:"Sep",deposit:420,withdrawal:180},{month:"Oct",deposit:380,withdrawal:220},
  {month:"Nov",deposit:490,withdrawal:150},{month:"Tue",deposit:340,withdrawal:280},
  {month:"May",deposit:460,withdrawal:190},{month:"Jun",deposit:500,withdrawal:240},
  {month:"Jul",deposit:420,withdrawal:160},{month:"Aug",deposit:480,withdrawal:200},
];
  ...
  return (
    <div onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)}
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
        ...
        <button onClick={()=>onUnlink(acc.id)} style={{width:28,height:28,borderRadius:8,border:`1px solid ${C.line}`,background:"transparent",color:C.muted,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}} title="Unlink">
          <Trash2 size={12}/>
        </button>
      </div>
      <p style={{color:C.muted,fontSize:11,margin:"0 0 4px",textTransform:"uppercase",letterSpacing:0.5}}>Balance</p>
      <p style={{color:C.primary,fontWeight:800,fontSize:22,margin:0,letterSpacing:-0.5}}>{currency}{acc.balance.toLocaleString("en-US",{minimumFractionDigits:2})}</p>
...
function LinkNewCard({onLink}:{onLink:()=>void}) {
  ...
  return (
    <button onClick={onLink} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)}
      ...
      <div style={{width:36,height:36,borderRadius:"50%",background:hovered?"#8FFFD618":C.hover,border:`1px solid ${hovered?"#8FFFD644":C.line}`,display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.2s"}}>
...
const CustomTooltip=({active,payload,label}:any)=>{
  ...
  return(
    <div style={{background:"var(--color-card)",border:"1px solid var(--color-line)",borderRadius:10,padding:"10px 14px",fontSize:12}}>
      ...
      {payload.map((p:any)=>(
        <div key={p.name} style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
          ...
          <span style={{color:"var(--color-primary)",fontWeight:600}}>${p.value}</span>
        </div>
      ))}
    </div>
  );
...
export default function WalletPage() {
  ...
  const handleUnlink=(id:string)=>setAccounts(p=>p.filter(a=>a.id!==id));
  const filteredTx=RECENT_TX.filter(t=>activeFilter==="all"||t.type===activeFilter);
    ...
    <div style={{padding:"24px 32px",maxWidth:1100,margin:"0 auto",fontFamily:"'Geist','Inter',sans-serif",background:C.page,minHeight:"100vh"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:28}}>
        <div>
          ...
          <p style={{color:C.muted,fontSize:12,margin:"4px 0 0"}}>Manage your linked bank accounts and fund transfers</p>
        </div>
      ...
      <div style={{background:C.card,border:`1px solid ${C.line}`,borderRadius:16,padding:"22px 28px",marginBottom:24,display:"flex",alignItems:"center",justifyContent:"space-between",position:"relative",overflow:"hidden"}}>
        ...
        <div>
          <p style={{color:C.muted,fontSize:11,textTransform:"uppercase",letterSpacing:1,margin:"0 0 6px"}}>Total Linked Balance</p>
          <p style={{color:C.primary,fontWeight:800,fontSize:36,margin:0,letterSpacing:-1}}>{currency}{totalBalance.toLocaleString("en-US",{minimumFractionDigits:2})}</p>
          <p style={{color:"#22c55e",fontSize:12,margin:"6px 0 0",fontWeight:500}}>+{currency}280.00 this month</p>
        </div>
        <div style={{display:"flex",gap:10}}>
          <button style={{display:"flex",alignItems:"center",gap:8,padding:"10px 18px",background:"#8FFFD618",borderRadius:10,border:"1px solid #8FFFD633",cursor:"pointer",color:"#8FFFD6",fontWeight:600,fontSize:13}}>
            <ArrowDownLeft size={14}/> Deposit
          </button>
          <button style={{display:"flex",alignItems:"center",gap:8,padding:"10px 18px",background:C.hover,borderRadius:10,border:`1px solid ${C.line}`,cursor:"pointer",color:C.muted,fontWeight:600,fontSize:13}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              {[{color:"#8FFFD6",label:"Deposit"},{color:"#6366f1",label:"Withdrawal"}].map(l=>(
                <div key={l.label} style={{display:"flex",alignItems:"center",gap:5}}>
                  <div style={{width:8,height:8,borderRadius:2,background:l.color}}/><span style={{color:C.muted,fontSize:11}}>{l.label}</span>
                </div>
              ))}
            </div>
          </div>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={FUNDING_HISTORY} barGap={4} margin={{top:0,right:0,left:-20,bottom:0}}>
              ...
              <Bar dataKey="deposit" name="Deposit" radius={[4,4,0,0]} maxBarSize={18}>
                {FUNDING_HISTORY.map((_,i)=><Cell key={i} fill={i%3===0?"#8FFFD6":i%3===1?"#6366f1":"#f59e0b"}/>)}
              </Bar>
              <Bar dataKey="withdrawal" name="Withdrawal" radius={[4,4,0,0]} maxBarSize={18}>
                {FUNDING_HISTORY.map((_,i)=><Cell key={i} fill={i%3===0?"#8FFFD655":i%3===1?"#6366f155":"#f59e0b55"}/>)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
            ...
            <div style={{display:"flex",gap:4}}>
              {(["all","deposit","withdrawal"] as const).map(f=>(
                <button key={f} onClick={()=>setActiveFilter(f)} style={{padding:"4px 10px",borderRadius:6,border:"none",cursor:"pointer",fontSize:10,fontWeight:600,textTransform:"capitalize",background:activeFilter===f?"#8FFFD618":"transparent",color:activeFilter===f?"#8FFFD6":C.muted}}>{f}</button>
              ))}
            </div>
          </div>
            ...
            {filteredTx.map(tx=>(
              <div key={tx.id} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 12px",borderRadius:10,background:C.hover,border:`1px solid ${C.line}`}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <p style={{color:C.muted,fontSize:10,margin:"2px 0 0",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{tx.account} · {tx.date}</p>
                </div>
                <div style={{textAlign:"right"}}>
                  <p style={{color:tx.type==="deposit"?"#22c55e":"#ef4444",fontWeight:700,fontSize:13,margin:0}}>{tx.type==="deposit"?"+":"-"}{currency}{tx.amount.toLocaleString()}</p>
                  <p style={{fontSize:9,margin:"2px 0 0",fontWeight:600,textTransform:"uppercase",color:tx.status==="completed"?"#22c55e":"#f59e0b"}}>{tx.status}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:50,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowLinkModal(false)}>
          <div style={{background:C.card,border:`1px solid ${C.line}`,borderRadius:16,padding:"28px 32px",width:420}} onClick={e=>e.stopPropagation()}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
              <div style={{width:36,height:36,borderRadius:10,background:"#8FFFD618",border:"1px solid #8FFFD633",display:"flex",alignItems:"center",justifyContent:"center"}}><Link2 size={16} color="#8FFFD6"/></div>
              <h3 style={{color:C.primary,fontWeight:700,fontSize:16,margin:0}}>Link New Account</h3>
            </div>
            {[{label:"Bank Name",placeholder:"e.g. Chase, Wells Fargo"},{label:"Account Number",placeholder:"Enter account number"},{label:"Routing Number",placeholder:"Enter routing number"}].map(({label,placeholder})=>(
