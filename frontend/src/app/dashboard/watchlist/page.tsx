...
import { Star, Plus, Search, Trash2, Bell, BellOff, TrendingUp, TrendingDown, ArrowUpRight, RefreshCw } from "lucide-react";

interface WatchItem {
  symbol:string;name:string;price:number;change:number;changePct:number;volume:string;
  high:number;low:number;color:string;bg:string;letter:string;
  alerted:boolean;alertPrice:number|null;sparkline:{v:number}[];sector:string;
}
...
const LOGO_DOMAINS:Record<string,string>={
  ...
  NFLX:"netflix.com",RELIANCE:"ril.com",TCS:"tcs.com",INFY:"infosys.com",HDFCBANK:"hdfcbank.com",
};

function makeSparkline(base:number,up:boolean){
  const pts:{v:number}[]=[];let v=base*(up?0.92:1.08);
  for(let i=0;i<12;i++){v+=(Math.random()-(up?0.35:0.65))*base*0.015;pts.push({v:+v.toFixed(2)});}
  pts.push({v:base});return pts;
}
...
const C={page:"var(--color-page)",card:"var(--color-card)",line:"var(--color-line)",primary:"var(--color-primary)",muted:"var(--color-muted)",hover:"var(--color-surface-hover)"};

function StockAvatar({symbol,color,bg,letter,size=36}:{symbol:string;color:string;bg:string;letter:string;size?:number}){
  const [err,setErr]=useState(false);
  const clean=symbol.replace(".BSE","").replace("/","");
  const domain=LOGO_DOMAINS[clean];
  return(
    <div style={{width:size,height:size,borderRadius:"50%",background:bg,border:`1px solid ${color}33`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:size*0.3,fontWeight:700,color,flexShrink:0,overflow:"hidden"}}>
      {domain&&!err?<img src={`https://icons.duckduckgo.com/ip3/${domain}.ico`} alt={symbol} width={size*0.6} height={size*0.6} onError={()=>setErr(true)} style={{objectFit:"contain",borderRadius:"50%"}}/>:(letter||symbol.charAt(0))}
    </div>
  );
}
  ...
  return(
    <ResponsiveContainer width={80} height={36}>
      <AreaChart data={data} margin={{top:2,right:0,bottom:0,left:0}}>
        ...
        <Area type="monotone" dataKey="v" stroke={up?"#22c55e":"#ef4444"} strokeWidth={1.5} fill={`url(#wsg${up?"u":"d"})`} dot={false} isAnimationActive={false}/>
      </AreaChart>
    </ResponsiveContainer>
  );
}

function AlertModal({item,onClose,onSave}:{item:WatchItem;onClose:()=>void;onSave:(p:number)=>void}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:50,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
      <div style={{background:"var(--color-card)",border:"1px solid var(--color-line)",borderRadius:16,padding:"28px 32px",width:360}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
          <div style={{width:36,height:36,borderRadius:10,background:"#f59e0b18",border:"1px solid #f59e0b33",display:"flex",alignItems:"center",justifyContent:"center"}}><Bell size={16} color="#f59e0b"/></div>
          <div>
            <h3 style={{color:C.primary,fontWeight:700,fontSize:15,margin:0}}>Set Price Alert</h3>
            <p style={{color:C.muted,fontSize:12,margin:"2px 0 0"}}>{item.symbol} · Current: {item.price}</p>
          </div>
        </div>
        <label style={{color:C.muted,fontSize:12,display:"block",marginBottom:6}}>Alert when price reaches</label>
        <input type="number" value={price} onChange={e=>setPrice(e.target.value)} placeholder={item.price.toString()}
        ...
        <div style={{display:"flex",gap:10,marginTop:20}}>
          ...
          <button onClick={()=>{onSave(parseFloat(price));onClose();}} style={{flex:1,padding:"11px 0",borderRadius:10,border:"none",background:"#f59e0b",color:"#0a0a0a",cursor:"pointer",fontWeight:700,fontSize:13}}>Set Alert</button>
        </div>
      </div>
    </div>
  );
...
export default function WatchlistPage(){
  ...
  useEffect(()=>{
    ...
    fetch("/api/watchlist",{headers:{Authorization:`Bearer ${token}`}})
      ...
      .then(data=>{if(data?.length>0)setItems(data);else setItems(DEFAULT_WATCHLIST[key]??DEFAULT_WATCHLIST["US"]);})
      .catch(()=>setItems(DEFAULT_WATCHLIST[key]??DEFAULT_WATCHLIST["US"]))
      .finally(()=>setLoading(false));
  },[key]);

  const removeItem=(symbol:string)=>setItems(p=>p.filter(i=>i.symbol!==symbol));
  const setAlert=(symbol:string,price:number)=>setItems(p=>p.map(i=>i.symbol===symbol?{...i,alerted:true,alertPrice:price}:i));
  ...
  const filtered=items.filter(i=>i.symbol.toLowerCase().includes(search.toLowerCase())||i.name.toLowerCase().includes(search.toLowerCase()));
    ...
    <div style={{padding:"24px 32px",maxWidth:1100,margin:"0 auto",fontFamily:"'Geist','Inter',sans-serif",background:C.page,minHeight:"100vh"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24}}>
        <div>
          ...
          <p style={{color:C.muted,fontSize:12,margin:"4px 0 0"}}>{market.flag} {market.label} · {items.length} symbols · {alertedCount} alerts active</p>
        </div>
        <div style={{display:"flex",gap:10}}>
          <button onClick={()=>setItems(DEFAULT_WATCHLIST[key]??[])} style={{width:36,height:36,borderRadius:10,border:`1px solid ${C.line}`,background:"transparent",color:C.muted,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><RefreshCw size={14}/></button>
          <button onClick={()=>setShowAdd(true)} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 18px",background:"#8FFFD6",borderRadius:10,border:"none",cursor:"pointer",color:"#0a0a0a",fontWeight:700,fontSize:13}}><Plus size={14}/> Add Symbol</button>
        </div>
      </div>

      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
        {[{label:"Watching",value:items.length,color:"#8FFFD6"},{label:"Gaining",value:gainers,color:"#22c55e"},{label:"Losing",value:losers,color:"#ef4444"},{label:"Alerts Set",value:alertedCount,color:"#f59e0b"}].map(({label,value,color})=>(
      ...
      <div style={{background:C.card,border:`1px solid ${C.line}`,borderRadius:14,overflow:"hidden"}}>
        ...
        {loading?(
          ...
          <div style={{padding:48,textAlign:"center"}}>
            ...
            <p style={{color:C.muted,fontSize:13}}>{search?`No results for "${search}"`:"Your watchlist is empty. Add a symbol to get started."}</p>
          </div>
        ):(
          filtered.map((item,i)=>{
                ...
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  ...
                  <div>
                    <div style={{display:"flex",alignItems:"center",gap:6}}>
                      <p style={{color:C.primary,fontWeight:600,fontSize:13,margin:0}}>{item.symbol}</p>
                      {item.alerted&&<span style={{fontSize:9,padding:"1px 5px",borderRadius:4,background:"#f59e0b18",color:"#f59e0b",border:"1px solid #f59e0b33",fontWeight:600}}>ALERT {item.alertPrice?`@ ${item.alertPrice}`:""}</span>}
                    </div>
                    <p style={{color:C.muted,fontSize:11,margin:"2px 0 0"}}>{item.name}</p>
                  </div>
                </div>
                <span style={{color:C.primary,fontSize:13,fontWeight:600}}>{currency}{item.price.toLocaleString("en-US",{minimumFractionDigits:item.price<10?4:2})}</span>
                <div style={{display:"flex",alignItems:"center",gap:5}}>
                  {isUp?<TrendingUp size={12} color="#22c55e"/>:<TrendingDown size={12} color="#ef4444"/>}
                  <span style={{color:isUp?"#22c55e":"#ef4444",fontSize:12,fontWeight:600}}>{isUp?"+":""}{item.changePct.toFixed(2)}%</span>
                </div>
                <span style={{color:C.muted,fontSize:12}}>{item.volume}</span>
                <div onClick={e=>e.stopPropagation()}><MiniSparkline data={item.sparkline} up={isUp}/></div>
                <button onClick={e=>{e.stopPropagation();router.push(`/dashboard/stock/${item.symbol}`);}} style={{width:30,height:30,borderRadius:8,border:`1px solid ${C.line}`,background:"transparent",color:C.muted,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><ArrowUpRight size={13}/></button>
                <button onClick={e=>{e.stopPropagation();setAlertItem(item);}} style={{width:30,height:30,borderRadius:8,border:`1px solid ${C.line}`,background:item.alerted?"#f59e0b18":"transparent",color:item.alerted?"#f59e0b":C.muted,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
                  {item.alerted?<Bell size={13}/>:<BellOff size={13}/>}
                </button>
                <button onClick={e=>{e.stopPropagation();removeItem(item.symbol);}} style={{width:30,height:30,borderRadius:8,border:`1px solid ${C.line}`,background:"transparent",color:C.muted,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><Trash2 size={13}/></button>
              </div>
            );
          })
        )}
      ...
      {showAdd&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:50,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowAdd(false)}>
          <div style={{background:C.card,border:`1px solid ${C.line}`,borderRadius:16,padding:"28px 32px",width:360}} onClick={e=>e.stopPropagation()}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
              <div style={{width:36,height:36,borderRadius:10,background:"#8FFFD618",border:"1px solid #8FFFD633",display:"flex",alignItems:"center",justifyContent:"center"}}><Star size={16} color="#8FFFD6"/></div>
            ...
            <input value={addSymbol} onChange={e=>setAddSymbol(e.target.value.toUpperCase())} onKeyDown={e=>e.key==="Enter"&&handleAdd()}
              placeholder={`e.g. ${key==="IN"?"RELIANCE":key==="CRYPTO"?"BTC":"AAPL"}`}
              style={{width:"100%",background:C.page,border:`1px solid ${C.line}`,borderRadius:8,color:C.primary,fontSize:14,padding:"10px 14px",outline:"none",boxSizing:"border-box"}} autoFocus/>
            <div style={{display:"flex",gap:10,marginTop:20}}>
              ...
              <button onClick={handleAdd} style={{flex:1,padding:"11px 0",borderRadius:10,border:"none",background:"#8FFFD6",color:"#0a0a0a",cursor:"pointer",fontWeight:700,fontSize:13}}>View &amp; Add</button>
            </div>
          </div>
        </div>
      )}
      {alertItem&&<AlertModal item={alertItem} onClose={()=>setAlertItem(null)} onSave={(p)=>setAlert(alertItem.symbol,p)}/>}
